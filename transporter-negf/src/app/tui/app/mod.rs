...
impl<'a> App {
                    ...
                    match self.state {
                        ...
                        _ => {
                            if let Some(file_to_run) = files_list_state.selected() {
                                ...
                                let tracker = std::sync::Arc::new(state::Tracker::new(file_to_run));
    ...
    pub(crate) fn running(&mut self, tracker: std::sync::Arc<state::Tracker>) {
...
pub(crate) async fn run_simulation(
    ...
    sender: tokio::sync::mpsc::Sender<state::Tracker>,
) {
    std::thread::sleep(std::time::Duration::from_secs(5));
    let new_state = state::Tracker::new_random();
    sender.send(new_state).await.unwrap();
    log::info!("Sent the state");
    std::thread::sleep(std::time::Duration::from_secs(5));
