...
mod test {
    ...
    fn inverse_fermi_integral_m05<T: Copy + RealField>(u: T) -> T {
        ...
        assert!(u > 0.);
        let u1 = 2.3441053212124923635;
        let u2 = 3.8475810464125087923;
        let u3 = 5.6270896934467898338;
        let u4 = 8.0747714790662749546;
        let u5 = 12.670348407162363013;
        if u <= u1 {
            let v = u1 - u;
            let r = u
                * (4.1455867063620057963e3
                    + v * (4.6492489228825891548e3
                        + v * (2.5468279472604635596e3
                            + v * (8.2617183361519822852e2
                                + v * (1.6523324342898456833e2
                                    + v * (1.9139098333899331728e1
                                        + v * (1.0000233523167968436e0)))))))
                / (1.8339260939384764384e3
                    + v * (3.6590141066361902419e3
                        + v * (3.3125828231919297460e3
                            + v * (1.7661058946629507641e3
                                + v * (5.9995878325365685481e2
                                    + v * (1.3022439202514782925e2
                                        + v * (1.6795762274957634668e1 + v)))))));
            r.ln()
        } else if u <= u2 {
            let y = u - u1;
            (1.0005640037689060269e5
                + y * (9.3795876901690924263e4
                    + y * (1.1968436775807236253e4
                        + y * (3.6056056623169881285e3
                            + y * (3.0032390254773869879e3
                                + y * (2.0666293152698467755e2
                                    + y * (4.6967188364979637811e1
                                        + y * (1.2655799929098327278e1))))))))
                / (6.0004232272321102537e4
                    + y * (9.4591460433134797911e3
                        + y * (-3.0260143411431122729e3
                            + y * (2.2053261399494976502e3
                                + y * (1.6664644949295151752e2
                                    + y * (-6.1135903669333744537e1
                                        + y * (1.7137481288783257727e1 - y)))))))
        } else if u <= u3 {
            let y = u - u2;
            (7.8077080328620324941e5
                + y * (7.4511980207693273717e5
                    + y * (4.8570139135090628839e5
                        + y * (2.1012609976203759489e5
                            + y * (6.5930590125900094802e4
                                + y * (1.6082333682678120185e4
                                    + y * (2.3070507238486040474e3
                                        + y * (2.1459577052537728303e2))))))))
                / (1.9761087937870631931e5
                    + y * (9.7473867500546756247e4
                        + y * (6.5697985373221757818e4
                            + y * (1.5810815030487697013e4
                                + y * (5.1437064634502537530e3
                                    + y * (5.4591421450702277492e2
                                        + y * (2.5466648701491294769e1 - y)))))))
        } else if u <= u4 {
            let y = u - u3;
            (9.723633697100386227e7
                + y * (1.0164545252921651718e8
                    + y * (5.610803657489166187e7
                        + y * (2.0533262113418584594e7
                            + y * (5.262211474540097266e6
                                + y * (9.659593698445202991e5
                                    + y * (1.1215569374855787353e5
                                        + y * (6.002807590319266831e3))))))))
                / (1.2112831832812397277e7
                    + y * (8.485660688557204553e6
                        + y * (3.6641826655801482580e6
                            + y * (1.0199175729180822690e6
                                + y * (1.8645751101280014065e5
                                    + y * (2.3351572064362269016e4
                                        + y * (3.6375418678884013347e1 - y)))))))
        } else if u <= u5 {
            let y = u - u4;
            (9.859217899528706965e10
                + y * (9.212482052867463736e10
                    + y * (4.659030699733763790e10
                        + y * (1.500219539387355430e10
                            + y * (3.429550982788585142e9
                                + y * (5.388776274849922981e8
                                    + y * (4.856263306490216609e7
                                        + y * (1.797949664146387803e6))))))))
                / (6.029507485001503472e9
                    + y * (4.1500445650217712266e9
                        + y * (1.7347612878358778140e9
                            + y * (4.2659950096560926778e8
                                + y * (7.803780012911421033e7
                                    + y * (7.194457928823351679e6
                                        + y * (-7.176409041887143411e1 + y)))))))
        } else {
            let wc = 25798.742392161875614;
            let t = wc / u.powi(4);
            let s = 1. - t;
            let r = (1.5170328199539480859e6
                + s * (1.7902383580145112376e5 + s * (2.9240858775000640610e3 + s)))
                / (t * (9.398803527483198144e2
                    + s * (1.1187685020374838766e2
                        + s * (1.9239024822840602874e0 + s * (2.3436621932409201918e-3)))));
            r.sqrt()
        }
    }
    ...
    fn numerical_electron_density_jacobian_matches_analytical() {
        ...
        for _ in 0..100 {
            ...
            let n = |phi| fermi_integral_05((ratio + phi) / denominator);
