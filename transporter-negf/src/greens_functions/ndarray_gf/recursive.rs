    ...
    T: RealField + Copy,
{
    ...
    let mut previous_hopping_element = T::from_real(hamiltonian.data()[nnz - 2]); // t_{i, i+1}
    for (idx, (element, row)) in diagonal
        .iter_mut()
        .rev()
        ...
        .enumerate()
    {
        ...
        let hopping_element = T::from_real(row.data()[num_in_row - 1]);
    ...
    <T as ComplexField>::RealField: Copy,
{
    ...
    // g_{N-1N-1}^{RR} = D_{N-1}^{-1}
    diagonal[nrows - 1] =
        T::one() / (T::from_real(energy - hamiltonian.data()[nnz - 1]) - self_energies.1);
        ...
        .enumerate()
    {
        let num_in_row = row.data().len();
        let hopping_element = T::from_real(row.data()[num_in_row - 1]);
        let diagonal = T::from_real(energy - row.data()[num_in_row - 2])
            - if idx == hamiltonian.nrows() - 2 {
                self_energies.1
            } else {
                T::zero()
    ...
    T: RealField + Copy,
{
    ...
    if row_index > nrows / 2 {
            ...
            .enumerate()
        {
            ...
            let hopping_element = -T::from_real(row.data()[row.len() - 1]);
            *element = -g_lr * hopping_element * previous;
            previous = *element;
        }
        Ok(row)
    } else {
        let mut row: Array1<Complex<T>> = Array1::zeros(nrows - row_index);
        ...
        {
            let row = hamiltonian.outer_view(row_index + idx - 1).unwrap();
            let hopping_element = -T::from_real(row.data()[0]);
...
{
    let nrows = hamiltonian.nrows();
    let mut right_column: Array1<T> = Array1::zeros(nrows);
    ...
    for (idx, (element, &left_diagonal_element)) in right_column
        ...
        .enumerate()
    {
        let row = hamiltonian.outer_view(nrows - 2 - idx);
